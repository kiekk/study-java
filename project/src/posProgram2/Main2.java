...
class MainFrame extends JFrame{
	...
	boolean userLogin(String nickName,String pw) {
		Customer log_NickName = checkNickName(nickName);
		if(log_NickName != null) {
			if(userList.get(userList.indexOf(log_NickName)).getPw().equals(pw)) {
				return true;
			}else {
				return false;
			}
	...
	//회원 가입 
	boolean join(String std) {
		if(checkNickName(std) == null) {	
			return true;	
		}else {								
			return false;	
		} 
	...
	public MainFrame() {
		init();
		Admin admin = new Admin("admin","1111");
		setTitle("Cafe Main");
		...
		btnLogin.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				ok.addMouseListener(new MouseAdapter() {
					@Override
					public void mousePressed(MouseEvent e) {
						if(userLogin(tfNickName.getText(),tfPw.getText())) {
							JOptionPane.showMessageDialog(null, "로그인 성공");
							setVisible(false);
							new UserPageFrame(checkNickName(tfNickName.getText()));
						}else {
